	...
	tests := []struct {
		...
		},
		{
			...
			func(svc volumes.MountService, cs *volumes.CryptSetup, device string) error {
				return svc.FormatDisk(device, "ext4", volumes.XFSOpts{})
			},
			nil,
		},
		{
			...
			volumes.MountOpts{},
			func(svc volumes.MountService, cs *volumes.CryptSetup, device string) error {
				return svc.FormatDisk(device, "xfs", volumes.XFSOpts{})
			},
		...
		},
		{
			...
			func(svc volumes.MountService, cs *volumes.CryptSetup, device string) error {
				...
				return svc.FormatDisk(luksDevicePath, "ext4", volumes.XFSOpts{})
			},
			nil,
		},
		{
	...
	tests := []*struct {
		...
		},
		{
			...
			func(svc *volumes.LinuxMountService, disk string) error {
				return svc.FormatDisk(disk, "ext4", volumes.XFSOpts{})
			},
			"ext4",
		},
		{
			"xfs",
			func(svc *volumes.LinuxMountService, disk string) error {
				return svc.FormatDisk(disk, "xfs", volumes.XFSOpts{})
			},
			"xfs",
		},
		{
